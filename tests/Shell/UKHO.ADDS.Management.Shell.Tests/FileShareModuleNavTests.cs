...
using UKHO.ADDS.Management.Modules.FileShare;
using UKHO.ADDS.Management.Shell.Services;
using Xunit;

namespace UKHO.ADDS.Management.Shell.Tests;

public class FileShareModuleNavTests
{
    [Fact]
    public void WhenUserLacksFileshareuserRole_ThenFileSharePageIsNotInNavigation()
    {
        var module = new FileShareModule();
        ...
        Assert.DoesNotContain(Flatten(svc.Pages), p => p.Path == "/fileshare");
    }

    [Fact]
    public void WhenUserHasFileshareuserRole_ThenFileSharePageIsInNavigation()
    {
        var module = new FileShareModule();
        var principal = new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim(ClaimTypes.Role, "fileshareuser") }, "test"));
        var auth = new TestAuthenticationStateProvider(principal);
        var health = new ModuleHealthService(new NullLogger<ModuleHealthService>());

        var svc = new ModulePageService(new[] { module }, auth, health);

        Assert.Contains(Flatten(svc.Pages), p => p.Path == "/fileshare");
    }
