...
using UKHO.ADDS.Management.Modules.Developer;
...
{
    [Fact]
    public void WhenUserLacksDeveloperRole_ThenSecurePageIsNotInNavigation()
    {
        ...
        Assert.DoesNotContain(Flatten(svc.Pages), p => p.Path == "/developer/secure");
    }

    [Fact]
    public void WhenUserHasDeveloperRole_ThenSecurePageIsInNavigation()
    {
        var module = new DeveloperModule();
        var principal = new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim(ClaimTypes.Role, "developer") }, "test"));
        var auth = new TestAuthenticationStateProvider(principal);
        var health = new ModuleHealthService(new NullLogger<ModuleHealthService>());

        var svc = new ModulePageService(new[] { module }, auth, health);

        Assert.Contains(Flatten(svc.Pages), p => p.Path == "/developer/secure");
    }
